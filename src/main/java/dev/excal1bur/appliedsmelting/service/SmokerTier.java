...
/** ME Smoker tier. Ordinal order also defines upgrade-kit progression order. */
public enum SmokerTier {
    DEFAULT(AppliedSmeltingConfig.SMOKER_DEFAULT),
    MK1(AppliedSmeltingConfig.SMOKER_MK1),
    MK2(AppliedSmeltingConfig.SMOKER_MK2),
    MK3(AppliedSmeltingConfig.SMOKER_MK3);

    private final AppliedSmeltingConfig.TierValues values;

    SmokerTier(AppliedSmeltingConfig.TierValues values) {
        this.values = values;
    }
    ...
    public SmokerTier previousTier() {
        var all = values();
        var index = ordinal();
    ...
    public static SmokerTier fromSerializedName(String name) {
        for (var tier : values()) {
    ...
    public static SmokerTier fromKitLevel(int level) {
        return switch (level) {
            case 1 -> MK1;
            case 2 -> MK2;
            case 3 -> MK3;
